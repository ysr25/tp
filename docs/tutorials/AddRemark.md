...
/**
 * Changes the remark of an existing flashcard in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the flashcard identified "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the flashcard in the filtered flashcard list to edit the remark
     * @param remark of the flashcard to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.bagel.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `seedu.bagel.model.flashcard`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each flashcard.

Simply add the following to [`seedu.bagel.ui.PersonCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person flashcardToEdit = lastShownList.get(index.getZeroBased());
        Person editedFlashcard = new Person(flashcardToEdit.getName(), flashcardToEdit.getPhone(), flashcardToEdit.getEmail(),
     ...
     */
    private String generateSuccessMessage(Person flashcardToEdit) {
